...
import { Edit, Trash2, X, Plus, AlertCircle } from "lucide-react";
...
export default function UsersManagementPage() {
  ...
  // Form State & Zod Error State
  const [formData, setFormData] = useState<UserFormValues>({ name: "", email: "", role: "USER" });
  ...
  const openModal = (type: "add" | "edit" | "delete", user?: UserData) => {
    ...
    if (user) {
      ...
      setFormData({ name: user.name, email: user.email, role: user.role as "USER" | "ADMIN" });
    } else {
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          ...
          className="bg-[#0088FF] hover:bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-md transition-colors flex items-center gap-2 cursor-pointer"
        >
            ...
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-6 py-4 flex justify-center gap-3">
                  <button onClick={() => openModal("edit", user)} className="text-gray-400 hover:text-blue-600 p-1 border border-gray-200 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 cursor-pointer">
                    <Edit size={16} />
                  </button>
                  <button onClick={() => openModal("delete", user)} className="text-red-400 hover:text-red-600 p-1 border border-gray-200 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 cursor-pointer">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-[#1F2937] w-full max-w-md rounded-3xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-200 border border-gray-100 dark:border-gray-800">
            <button onClick={() => setIsModalOpen(false)} className="absolute right-6 top-6 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 cursor-pointer">
              <X size={24} />
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {modalType !== "delete" ? (
                <>
                  <div>
                    ...
                    <input 
                      ...
                      className={`w-full bg-gray-50 dark:bg-gray-800 border rounded-xl px-4 py-3 text-sm focus:outline-none text-gray-900 dark:text-white ${
                        errors.name ? "border-red-500" : "border-gray-200 dark:border-gray-700 focus:border-[#0C134F]"
                      }`} 
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className={`w-full bg-gray-50 dark:bg-gray-800 border rounded-xl px-4 py-3 text-sm focus:outline-none text-gray-900 dark:text-white ${
                        errors.email ? "border-red-500" : "border-gray-200 dark:border-gray-700 focus:border-[#0C134F]"
                      }`} 
                      placeholder="email@contoh.com" 
                    />
                    {errors.email && (
                      <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle size={12}/> {errors.email}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#0C134F]"
                    >
                ...
                <p className="text-gray-600 dark:text-gray-300">
                  Apakah Anda yakin ingin menghapus pengguna <span className="font-bold text-red-600 dark:text-red-400">{selectedUser?.name}</span>? Tindakan ini tidak dapat dibatalkan.
                </p>
              )}

              <div className="flex justify-end gap-3 pt-4 mt-6 border-t border-gray-100 dark:border-gray-800">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 rounded-full text-sm font-bold text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 cursor-pointer">
                  Batal
                </button>
                <button type="submit" className={`px-6 py-2.5 rounded-full text-sm font-bold text-white shadow-md cursor-pointer ${modalType === "delete" ? "bg-red-600 hover:bg-red-700" : "bg-[#0C134F] hover:bg-[#151f6e]"}`}>
